def rbf(x_1, x_2, sigma=1.):
    ...
    line = np.sum(x_2.numpy() ** 2, axis = 1)
    row = np.sum(x_1.numpy() ** 2, axis = 1).reshape(n_samples_1, 1)
    norm = line + row - 2 * (x_1 @ x_2.T).numpy()
    distances = np.exp(- norm / (2 * sigma ** 2)) ### YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.clamp(1 - labels * scores, 0), dim=0) ### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        '''
        return torch.matmul(x_1, x_2.T) ### YOUR CODE HERE
